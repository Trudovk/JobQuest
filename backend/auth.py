...
def register_user(email: str, password: str, first_name: str, last_name: str):
    ...
    db.get_db().execute("""
    INSERT INTO users ("email", "passhash", "salt", "first_name", "last_name")
    VALUES (?, ?, ?, ?, ?);
...
def authenticate_user(email: str, password: str):
    ...
    db.get_db().execute("UPDATE users SET session=? WHERE email=?;", (session, email))
    return session


def get_user_from_session(session: str):
    query = db.get_db().execute("SELECT * FROM users WHERE session=?;", (session))
    existing_user = query.fetchone()
